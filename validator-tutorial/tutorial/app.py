...
from wtforms.validators import DataRequired, Length, ValidationError, NoneOf

app = Flask(__name__)
app.config['SECRET_KEY']='fLWk(VYAhX2V-YBk%Y5+0K8t,ed}@' #Configure a secret key for CSRF protection.
...
class myForm(FlaskForm):
    username = StringField('Username', validators=[
        ...
        UserCheck(message="A word in your username is invalid.",banned = ['root','admin','sys']),
        CharacterCheck(message="A character in your username is invalid",banned = ['!','"','£','$']),
        Length(min=2,max=15)
        ])
